...
def procesar_imagen(ruta_imagen, escala=0.4):
    ...
    # Binarización con Otsu y morfología
    blur = cv2.GaussianBlur(img_gray, (5, 5), 0)
    _, imgbin = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    imgbin_inv = 255 - imgbin
    imgbin_morf = cv2.erode(imgbin_inv, np.ones((5, 5), np.uint8), iterations=2)
...
procesar_imagen('monedas.jpg', escala=0.3)
procesar_imagen('mi_moneda.jpg', escala=0.4)